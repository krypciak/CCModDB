    ...
    if (oldPkgDb) {
        for (const name in pkgDb) {
            if (!oldPkgDb[name]) {
                ...
                console.log(ccmod.title, ccmod.description)
                console.log([ccmod.id, ccmod.version, db.getStringFromLocalisedString(ccmod.title ?? ''), db.getStringFromLocalisedString(ccmod.description ?? '')].join('|'))
            }
        }
    }
}
